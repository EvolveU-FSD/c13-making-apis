...
describe("user data layer", () => {
  ...
  it("should require a full name", async () => {
    //execute/verify
    await expect(createUser("tonye", null, "InceptionU")).rejects.toThrow(
      "user validation failed: fullName: Path `fullName` is required."
    );
  });

  it("should require a username that is not an empty string", async () => {
    // execute/verify
    await expect(createUser("", "Tony Enerson", "InceptionU")).rejects.toThrow(
      "user validation failed: username: Path `username` is required."
    );
  });
  ...
  it("should require a full name that is not an empty string", async () => {
    //execute/verify
    await expect(createUser("tonye", "", "InceptionU")).rejects.toThrow(
      "user validation failed: fullName: Path `fullName` is required."
    );
  });
  ...
  it("should not create a user with a duplicate username", async () => {
    ...
    // execute
    await expect(
      createUser("tonye", "Tony Eggbert", "Cupcakes4Fun")
  ...
  it("should update a user", async () => {
    ...
    // assert
    expect(updated.fullName).toEqual("Tony Eggbert");
    expect(updated.companyName).toEqual("Cupcakes4Fun");
  ...
  it("should set and verify a password (and not expose the password)", async () => {
    ...
    expect(actual._id).toEqual(createdUser._id);
